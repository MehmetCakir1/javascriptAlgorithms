...
//   So the first letter is given the code 0, and
//    second is then assigned 1 if it is different to the first letter or 0 otherwise, and so on.
...
function wordPattern(word) {
    let result=""
    let arr=[]
    word.toLowerCase().split("").map((item)=>!arr.includes(item) ? arr.push(item) :null)
        for(let k=0;k<word.length;k++){
            result+=arr.indexOf(word[k].toLowerCase())+"."
        }
